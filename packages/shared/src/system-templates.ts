/**
 * Plantillas del sistema MatuMailer (compartidas API + dashboard seed).
 * Estilo WhatsApp-like: slug fijo + variables declaradas.
 */
function shell(body: string, footer = '© {{companyName}} · MatuMailer'): string {
  return `<!DOCTYPE html>
<html lang="es">
...
<tr><td style="height:8px;background:linear-gradient(90deg,{{primaryColor}},#f5e6b8)"></td></tr>
${body}
...
<tr><td style="padding:32px 40px 8px;text-align:center">
  <img src="{{logo}}" alt="{{companyName}}" width="120" style="max-width:120px;height:auto;margin:0 auto 16px;display:block"/>
</td></tr>
...
<tr><td style="padding:0 40px 48px;text-align:center">
  <a href="{{enlace}}" style="display:inline-block;padding:16px 36px;background:{{primaryColor}};color:#1c1917;font-size:16px;font-weight:600;text-decoration:none;border-radius:999px">Comenzar ahora</a>
</td></tr>`),
    variables: ['nombre', 'enlace', 'logo', 'companyName', 'primaryColor'],
  },
  {
...
<tr><td style="padding:0 40px 40px">
  <p style="margin:0;font-size:13px;color:#a8a29e">Si no solicitaste esto, ignora este correo.</p>
    ...
    variables: ['nombre', 'resetLink', 'companyName', 'primaryColor'],
  },
  {
    slug: 'notification',
    name: 'Notificación',
...
<tr><td style="padding:40px 40px 8px">
  <p style="margin:0;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:{{primaryColor}}">Notificación</p>
...
<tr><td style="padding:16px 40px 40px">
  <p style="margin:0;font-size:16px;line-height:1.7;color:#44403c">{{mensaje}}</p>
    ...
    variables: ['titulo', 'mensaje', 'companyName', 'primaryColor'],
  },
  {
    slug: 'campana',
...
<tr><td style="padding:48px 40px 16px;text-align:center">
  <img src="{{logo}}" alt="" width="72" style="max-width:72px;margin:0 auto 24px;display:block"/>
...
<tr><td style="padding:8px 40px 24px;text-align:center">
  <p style="margin:0;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.1em;color:{{primaryColor}}">{{titulo}}</p>
</td></tr>
<tr><td style="padding:0 40px 32px">
  <p style="margin:0;font-size:17px;line-height:1.75;color:#57534e;text-align:center">{{mensaje}}</p>
</td></tr>
<tr><td style="padding:0 40px 48px;text-align:center">
  <a href="{{enlace}}" style="display:inline-block;padding:16px 40px;background:{{primaryColor}};color:#1c1917;font-size:16px;font-weight:600;text-decoration:none;border-radius:999px">Ver más</a>
</td></tr>`),
    variables: [
      'primerNombre',
      'titulo',
